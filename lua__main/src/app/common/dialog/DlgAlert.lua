...
-- Description: 基础对话框(自适配高度) 宽度：500，顶部标题栏高度60，底部按钮层100，默认内容层高度210
...
--解析布局回调事件
function DlgAlert:onParseViewCallback( pView )
	-- body
	...
	self:updateViews()
	 --注册析构方法
    self:setDestroyHandler("DlgAlert",handler(self, self.onDlgAlertDestroy))
    -- 注册关闭对话框的消息
...
-- 修改控件内容或者是刷新控件数据
function DlgAlert:updateViews()
	-- body

end

--析构方法
function DlgAlert:onDlgAlertDestroy()
	-- 销毁关闭对话框的消息
...
-- _pView ：布局
-- _bHadBottomBtn
function DlgAlert:addContentView( _pView, _bHadBottomBtn )
	-- body
	if _pView then
...
--_anchorPoint: 锚点设置
function DlgAlert:setContent(_content, _sColor, _nFontSize, _nConWidth, _nConHeight ,_anchorPoint)
	-- body
...
-- _nConHeight: 文字内容显示的区域
--_anchorPoint: 锚点设置
function DlgAlert:setContentLetter(_content, _sColor, _nFontSize, _nConWidth, _nConHeight ,_anchorPoint)
	-- body
	if not _content then
	    return
	end
...
-- 设置标题
function DlgAlert:setTitle(_title)
    if not _title then
    	return
    end
...
-- 设置左边handler
function DlgAlert:setLeftHandler(_handler)
    self._nLeftHandler = _handler
end

-- 设置右边handler
function DlgAlert:setRightHandler(_handler)
    self._nRightHandler = _handler
end

-- 取消左边按钮事件
function DlgAlert:onLeftClicked(pView)
...
-- 设置左右按钮层的高度
function DlgAlert:setBtnLayHeight(_height)
	-- body
...
-- 确定右边按钮事件
function DlgAlert:onRightClicked(pView)
...
--设置左边无效回调
function DlgAlert:setLeftDisabledHandler(_handler )
	-- body
	self._nLeftDisHandler = _handler
end

--设置左边无效回调
function DlgAlert:setRightDisabledHandler(_handler )
	-- body
	self._nRightDisHandler = _handler
end

-- 取消左边无效按钮事件
function DlgAlert:onLeftDisabledClicked(pView)
...
-- 确定右边无效按钮事件
function DlgAlert:onRightDisabledClicked(pView)
    if self._nRightDisHandler then
        self._nRightDisHandler()
    end
end

function DlgAlert:setCloseHandler(_handler)
	if _handler then
...
-- 关闭点击
function DlgAlert:onCloseClicked(pView)
...
-- 关闭对话框
function DlgAlert:closeAlertDlg()
	self:closeDlg(false)
end

-- 关闭对话框
function DlgAlert:onCloseDlg( sMsgName, pMsgObj )
...
--获得左边按钮
function DlgAlert:getLeftButton( )
	-- body
	return self.pBtnLeft
end

--获得右边按钮
function DlgAlert:getRightButton( )
	-- body
	return self.pBtnRight
end

-- 只有确定选项
function DlgAlert:setOnlyConfirm(_sText)
...
function DlgAlert:getOnlyConfirmButton(_nType,_sText)
...
--设置确定按钮高度
function DlgAlert:setOnlyConfirmBtnHeight(_height)
	-- body
	self.pLayRight:setPositionY(_height)
end

--设置按钮样式
function DlgAlert:setOnlyConfirmBtn( _nType )
	-- body
...
--设置左边按钮 文字内容
--_sText 按钮内容
function DlgAlert:setLeftBtnText(_sText)
	local sText = _sText
	if sText == nil then
		return
	end
...
--设置右边按钮 文字内容
--_sText 按钮内容
function DlgAlert:setRightBtnText(_sText)
	local sText = _sText
	if sText == nil then
		return
	end
...
--设置左边样式
function DlgAlert:setLeftBtnType( _nType )
	-- body
	self.pBtnLeft:updateBtnType(_nType)
end

--设置右边样式
function DlgAlert:setRightBtnType( _nType )
	-- body
	self.pBtnRight:updateBtnType(_nType)
end

--设置左边按钮是否有效
function DlgAlert:setLeftBtnEnabled(_bEnabled)
	self.pBtnLeft:setBtnEnable(_bEnabled)
end

--设置右边按钮是否有效
function DlgAlert:setRightBtnEnabled(_bEnabled)
	self.pBtnRight:setBtnEnable(_bEnabled)
end

--设置背景框为透明
function DlgAlert:setContentBgTransparent()
	-- body
	self.pLayContent:setBackgroundImage("ui/daitu.png")
end

--移除底部
function DlgAlert:setBottomBtnVisible(_bVisiable)
	-- 移除底部
