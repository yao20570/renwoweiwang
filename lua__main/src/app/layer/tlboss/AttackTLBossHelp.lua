...
-- updatetime: 2018-02-09 13:46:00
-- Description: 限时Boss界面
-----------------------------------------------------
local MCommonView = require("app.common.MCommonView")
local WorldHelpFunc = require("app.layer.worldhelp.WorldHelpFunc")
local AttackTLBossHelp = class("AttackTLBossHelp", function()
	return MCommonView.new(TYPE_LAYER.LAYER_MLAYER)
end)

function AttackTLBossHelp:ctor( pParent )
	self:myInit(pParent)
	parseView("lay_help_tlboss", handler(self, self.onParseViewCallback))
end

--读入控件后,初始化参数值
function AttackTLBossHelp:myInit(pParent)
...
--解析界面回调
function AttackTLBossHelp:onParseViewCallback( pView )
	self.pView = pView
	...
	--注册析构方法
	self:setDestroyHandler("AttackTLBossHelp", handler(self, self.onAtkHelpLayerDestroy))
end
function AttackTLBossHelp:setupViews(  )
	--内容层
	...
	self.pLbEnemy:setString(getConvertedStr(3, 10800))
...
function AttackTLBossHelp:updateViews()
	self:playAnim()
end


function AttackTLBossHelp:setLjShow( show )
...
--播放动画
function AttackTLBossHelp:playAnim(  )
	self:stopAllActions()
...
function AttackTLBossHelp:playStepOne()
	--画行军路线
...
function AttackTLBossHelp:setArmFlipped( _bFlip,_nRotate,_sName )
...
function AttackTLBossHelp:playSwordAnim()
...
--添加一条行军路线
--_nType：1：前进  2：返回
function AttackTLBossHelp:addLine(_nType,_nLineType)
...
--释放行军路线
function AttackTLBossHelp:releaseLine( )
...
--播放返回特效
function AttackTLBossHelp:playBackAnim()
	self.pHero:setVisible(true)
...
function AttackTLBossHelp:onResume(  )
	--刷新监听
...
--移动起来
function AttackTLBossHelp:onLineMove( )
...
function AttackTLBossHelp:onPause(  )
	
end
-- 析构方法
function AttackTLBossHelp:onAtkHelpLayerDestroy(  )
