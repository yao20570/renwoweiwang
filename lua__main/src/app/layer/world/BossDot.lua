...
local BossDot = class("BossDot",function ( )
...
function BossDot:ctor( pWorldLayer, pImgDot, pClickNode)
...
--解析界面回调
function BossDot:onParseViewCallback( pView )
	...
	--注册析构方法
    self:setDestroyHandler("BossDot",handler(self, self.onBossDotDestroy))
end

function BossDot:onBossDotDestroy(  )
	self:onPause()
end

function BossDot:onResume(  )
	self:regMsgs()
end

function BossDot:onPause(  )
	self:unregMsgs()
	unregUpdateControl(self)
end

function BossDot:regMsgs( )
end

function BossDot:unregMsgs( )
end

function BossDot:setupViews(  )
...
--设置服务器数据
--tData:ViewDotMsg
function BossDot:setData( tData )
	self.tData = tData
	self.tBossConf = nil
	if tData then
		self.tBossConf = getAwakeBossData(tData.nBossLv, Player:getWuWangDiff())
	end
...
--获取数据
function BossDot:getData(  )
	return self.tData
end

function BossDot:getDotKey()
...
--更新
function BossDot:updateViews(  )
	if not self.tData then
		return
	end

	--防止重复刷新
	if self.nBossLv ~= self.tData.nBossLv then
		self.nBossLv = self.tData.nBossLv
		local tBossConf = self.tBossConf
		if tBossConf then
			...
			--名字
			self.pBbName:setString(tBossConf.name)
			...
			--Boss图标
			-- local pImg = WorldFunc.getBossIconOfContainer(self.pLayIcon, self.nBossLv)
			-- if pImg then
			...
			-- 	end
			-- end
			local tBossData = getAwakeBossData(self.nBossLv, Player:getWuWangDiff())
			if tBossData then
...
--国战特效显示
function BossDot:showCityWarEffect( bIsShow )
	--减少刷新
...
--更新是否显示进攻特效
function BossDot:updateAtkEffect(  )
	if not self.tData then
		return
	end
...
--判断是否点中召唤气泡
--屏幕坐标
function BossDot:checkIsClickedWar( fWorldX, fWorldY )
	-- --是否点击中战
...
--隐藏
function BossDot:setVisibleEx( bIsShow )
	if bIsShow then
		--开始更新
		regUpdateControl(self, handler(self, self.updateCd))
	else
...
function BossDot:delViewDotMsg( )
	--清空数据
	if self.tData then
