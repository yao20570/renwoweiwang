-----------------------------------------------------
-- author: wangxs
-- updatetime:  2017-05-08 18:01:58 星期一
-- Description: 兵营
-----------------------------------------------------
local ItemCamp = require("app.layer.camp.ItemCamp")
...
local DlgCamp = class("DlgCamp", function()
	-- body
	return DlgBase.new(e_dlg_index.camp)
end)

--_nBuidlId：兵营id
--_bNewGuide:是否是新手任务
function DlgCamp:ctor( _nBuidlId, _bNewGuide )
	-- body
	self:myInit()
	self.nBuildId = _nBuidlId or self.nBuildId
	self.bNewGuide = _bNewGuide
	self.tBuildInfo = Player:getBuildData():getBuildById(self.nBuildId)
...
--初始化成员变量
function DlgCamp:myInit(  )
	-- body
	self.nBuildId 			= 		e_build_ids.infantry    --兵营id
	self.tBuildInfo 		= 		nil 					--兵营数据
...
--解析布局回调事件
function DlgCamp:onParseViewCallback( pView )
	-- body
	...
	--注册析构方法
	self:setDestroyHandler("DlgCamp",handler(self, self.onDlgCampDestroy))
end

--初始化控件
function DlgCamp:setupViews( )
	-- body
	--设置标题
	self:setNameAndLv()
	--头顶横条(banner)
	...
	if self.nBuildId == e_build_ids.infantry then   --步兵
	...
	elseif self.nBuildId == e_build_ids.sowar then  --骑兵
	...
	elseif self.nBuildId == e_build_ids.archer then --弓兵
...
--粮草消耗刷新
function DlgCamp:updateFoodCost(sMsgName, pMsgObj)
	-- body
...
-- 析构方法
function DlgCamp:onDlgCampDestroy(  )
	-- body
	self:onPause()
end

-- 注册消息
function DlgCamp:regMsgs( )
	-- body
	-- 注册兵营士兵招募队列刷新消息
	...
	-- 注册募兵消耗粮草消息
	regMsg(self, ghd_refresh_camp_recruit, handler(self, self.updateFoodCost))
	--物品变化刷新
	regMsg(self, gud_refresh_baginfo, handler(self, self.updateViews))
end

-- 注销消息
function DlgCamp:unregMsgs(  )
	-- body
	-- 销毁兵营士兵招募队列刷新消息
	...
	-- 注销募兵消耗粮草消息
	unregMsg(self, ghd_refresh_camp_recruit)
	unregMsg(self, gud_refresh_baginfo)
end


--暂停方法
function DlgCamp:onPause( )
	-- body
	self:unregMsgs()
...
--刷新队列数据
function DlgCamp:refreshLvItem(  )
	-- body
	--listview
	...
	if self.bIsRecuiting then
		...
		if self.pRecruitingItem == nil then
			self.pRecruitingItem = ItemCamp.new()
...
function DlgCamp:refreshScrollPos(  )
	-- body
	local nItemCnt = #self.tMadingLists
...
function DlgCamp:onActionClicked( _data )
	-- body
	if _data then
		if _data.nType == e_camp_item.free then --可招募
			local nMinute = _data.nCurSD / 60 --转化为分钟
			if not self.nFoodCost then
				self.nFoodCost = self.tBuildInfo:getNeedCostFood(self.tBuildInfo.sTid, _data.nCurNum)
			end
			--募兵需要消耗粮食少于拥有的数量
			if self.nFoodCost < Player:getPlayerInfo().nFood then
...
--请求数据回调
function DlgCamp:onResponse( __msg, __oldMsg )
	-- body
...
--增益点击事件
function DlgCamp:onZyClicked( pView )
	-- body
...
--季节点击事件
function DlgCamp:onJjClicked( pView )
	-- body
	print("增益")
	local tObject = {
...
-- 取消左边无效按钮事件
function DlgCamp:onLeftDisabledClicked(pView)
    print("取消左边无效按钮事件")
end

-- 确定右边无效按钮事件
function DlgCamp:onRightDisabledClicked(pView)
    TOAST(getConvertedStr(1, 10157))
end

-- 取消左边按钮事件
function DlgCamp:onLeftClicked(pView)
	local nVipGifyLv = 0 --兵种购买的对应VIP礼包等级
    local tOb = {}
    tOb.nType = e_dlg_index.shop
    if self.nBuildId == e_build_ids.infantry then
    	nVipGifyLv = getArmyVipLvLimit(e_id_item.bbgm)
    	tOb.nGoodsId = e_resdata_ids.bb
    elseif self.nBuildId == e_build_ids.sowar  then
    	nVipGifyLv = getArmyVipLvLimit(e_id_item.qbgm)
    	tOb.nGoodsId = e_resdata_ids.qb
    elseif self.nBuildId == e_build_ids.archer then
...
-- 确定右边按钮事件
function DlgCamp:onRightClicked(pView)
...
--招募队列数据刷新消息
function DlgCamp:onRefreshDatas( sMsgName, pMsgObj )
	-- body
	if pMsgObj then
...
function DlgCamp:onRefreshBuff()
	-- body
	self:initDatas()
	self:updateViews()
	self.bNewGuide = nil
...
function DlgCamp:getRecuitRedNum(  )
	-- body
	local nNum = 0
	--功能开启而且次数不超过2次
