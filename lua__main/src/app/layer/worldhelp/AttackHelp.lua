...
local AttackHelp = class("AttackHelp", function()
	return MCommonView.new(TYPE_LAYER.LAYER_MLAYER)
end)

function AttackHelp:ctor(_parent)
	self:myInit()
	self.pParent = _parent
	parseView("lay_help_1", handler(self, self.onParseViewCallback))
end

--读入控件后,初始化参数值
function AttackHelp:myInit()
	self.pParent 				= 		nil 			--父层
	self.pHero 					=		nil 
	self.pArm 					= 		nil 
	self.pLine 					= 		nil 
...
--解析界面回调
function AttackHelp:onParseViewCallback( pView )
	self.pView = pView
	...
	--注册析构方法
	self:setDestroyHandler("AttackHelp", handler(self, self.onAtkHelpLayerDestroy))
end
function AttackHelp:setupViews(  )
	--内容层
	self.pLayContent 	= 			self:findViewByName("middle")
	--城池图片
	self.pImgCity 		= 			self.pView:findViewByName("img_my")
	self.pImgCity:setScale(0.26)
	--乱军图片
	...
	self.pLbEnemy:setString(getConvertedStr(8, 10028))
...
function AttackHelp:updateViews()
	self:playAnim()
end

function AttackHelp:setLjShow(show)
...
function AttackHelp:playStepOne()
	--画行军路线
...
function AttackHelp:setArmFlipped( _bFlip,_nRotate,_sName )
...
function AttackHelp:playSwordAnim()
...
--添加一条行军路线
--_nType：1：前进  2：返回
function AttackHelp:addLine(_nType,_nLineType)
...
--释放行军路线
function AttackHelp:releaseLine( )
...
--播放返回特效
function AttackHelp:playBackAnim()
	self.pHero:setVisible(true)
...
--移动起来
function AttackHelp:onLineMove( )
...
function AttackHelp:onPause(  )
	
end
-- 析构方法
function AttackHelp:onAtkHelpLayerDestroy(  )
