...
local DataForgeRank = class("DataForgeRank", function()
	return Activity.new(e_id_activity.forgerank) 
end)

--创建自己(方便管理)
tActivityDataList[e_id_activity.forgerank] = function (  )
	return DataForgeRank.new()
end

function DataForgeRank:ctor()
	-- body
   self:myInit()
end


function DataForgeRank:myInit( )
 	self.tAs = {}				--已领取奖励			
 	self.tConfs = {}			--配置信息
...
-- 读取服务器中的数据
function DataForgeRank:refreshDatasByServer( _tData )
	--dump(_tData,"数据",20)
	if not _tData then
	 	return
	end
...
-- 获取红点方法
function DataForgeRank:getRedNums(_awardsTip)
	local nNums = 0
...
--获取奖励配置数据
function DataForgeRank:getPrizeConfs(  )
	-- body
	return self.tConfs
end
--是否开始领奖
function DataForgeRank:isStartGetPrize(  )
	-- body
...
--获取奖励配置信息
function DataForgeRank:getRankPrizeDatas(  )
	-- body
	local tTable = {}
...
--获取奖品状态
function DataForgeRank:getPrizeStatus( Index )
	-- body
...
function DataForgeRank:updateRankPrizeStatus( )
	-- body
	for k, v in pairs(self.tConfs) do
...
--是否已经领取奖励
function DataForgeRank:isHaveGetPrize(_index)
	-- body
	if not _index then
		return false
	end
...
--是否在奖励范围内
function DataForgeRank:isInGetPrizeRange( _index )
	-- body
...
--根据排名确定等级
function DataForgeRank:getClassifyByRank( _nRank )
	-- body
...
--获取当前玩家的奖励档次
function DataForgeRank:getMyPrizeClassify()
	-- body
	return self:getClassifyByRank(self.nHz) 
end

--活动结算时间
function DataForgeRank:getBalanceTimeStr(  )
	-- body	
