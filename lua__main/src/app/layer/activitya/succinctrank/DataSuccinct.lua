...
local DataSuccinct = class("DataSuccinct", function()
	return Activity.new(e_id_activity.succinctrank) 
end)

--创建自己(方便管理)
tActivityDataList[e_id_activity.succinctrank] = function (  )
	return DataSuccinct.new()
end

function DataSuccinct:ctor()
	-- body
   self:myInit()
end


function DataSuccinct:myInit( )
 	self.tAs = {}				--已领取奖励			
 	self.tConfs = {}			--配置信息
...
-- 读取服务器中的数据
function DataSuccinct:refreshDatasByServer( _tData )
	--dump(_tData,"数据",20)
	if not _tData then
	 	return
	end
	...
	self.nFm = _tData.t or self.nFm -- 洗练次数
...
-- 获取红点方法
function DataSuccinct:getRedNums(_awardsTip)
	local nNums = 0
...
--获取奖励配置数据
function DataSuccinct:getPrizeConfs(  )
	-- body
	return self.tConfs
end
--是否开始领奖
function DataSuccinct:isStartGetPrize(  )
	-- body
...
--获取奖励配置信息
function DataSuccinct:getRankPrizeDatas(  )
	-- body
	local tTable = {}
...
--获取奖品状态
function DataSuccinct:getPrizeStatus( Index )
	-- body
...
function DataSuccinct:updateRankPrizeStatus( )
	-- body
	for k, v in pairs(self.tConfs) do
...
--是否已经领取奖励
function DataSuccinct:isHaveGetPrize(_index)
	-- body
	if not _index then
		return false
	end
...
--是否在奖励范围内
function DataSuccinct:isInGetPrizeRange( _index )
	-- body
...
--根据排名确定等级
function DataSuccinct:getClassifyByRank( _nRank )
	-- body
...
--获取当前玩家的奖励档次
function DataSuccinct:getMyPrizeClassify()
	-- body
	return self:getClassifyByRank(self.nHz) 
end
--活动结算时间
function DataSuccinct:getBalanceTimeStr(  )
	-- body	
