...
local DataIronStore = class("DataIronStore", function()
	return Activity.new(e_id_activity.ironstore) 
end)

--创建自己(方便管理)
tActivityDataList[e_id_activity.ironstore] = function (  )
	return DataIronStore.new()
end

function DataIronStore:ctor()
	-- body
   self:myInit()
end


function DataIronStore:myInit( )
 	self.tAs = {}				--已领取奖励			
 	self.tConfs = {}			--配置信息
...
-- 读取服务器中的数据
function DataIronStore:refreshDatasByServer( _tData )
	--dump(_tData,"数据",20)
	if not _tData then
	 	return
	end
...
-- 获取红点方法
function DataIronStore:getRedNums(_awardsTip)
	local nNums = 0
...
--获取奖励配置数据
function DataIronStore:getPrizeConfs(  )
	-- body
	return self.tConfs
end
--是否开始领奖
function DataIronStore:isStartGetPrize(  )
	-- body
...
--获取奖励配置信息
function DataIronStore:getRankPrizeDatas(  )
	-- body
	local tTable = {}
...
--获取奖品状态
function DataIronStore:getPrizeStatus( Index )
	-- body
...
function DataIronStore:updateRankPrizeStatus( )
	-- body
	for k, v in pairs(self.tConfs) do
...
--是否已经领取奖励
function DataIronStore:isHaveGetPrize(_index)
	-- body
	if not _index then
		return false
	end
...
--是否在奖励范围内
function DataIronStore:isInGetPrizeRange( _index )
	-- body
...
--根据排名确定等级
function DataIronStore:getClassifyByRank( _nRank )
	-- body
...
--获取当前玩家的奖励档次
function DataIronStore:getMyPrizeClassify()
	-- body
	return self:getClassifyByRank(self.nHz) 
end

--活动结算时间
function DataIronStore:getBalanceTimeStr(  )
	-- body	
