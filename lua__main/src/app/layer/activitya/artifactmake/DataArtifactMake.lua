-- DataArtifactMake.lua
---------------------------------------------
-- Author: dshulan
-- Date: 2018-02-27 17:04
-- 神器升级数据
---------------------------------------------
local Activity = require("app.data.activity.Activity")
local ArtifactUpConfVo = require("app.layer.activitya.artifactmake.ArtifactUpConfVo")

local DataArtifactMake = class("DataArtifactMake", function()
	return Activity.new(e_id_activity.artifactmake) 
end)

--创建自己(方便管理)
tActivityDataList[e_id_activity.artifactmake] = function (  )
	return DataArtifactMake.new()
end

function DataArtifactMake:ctor()
	-- body
   self:myInit()
end


function DataArtifactMake:myInit( )
...
-- 读取服务器中的数据
function DataArtifactMake:refreshDatasByServer( _tData )
	-- dump(_tData,"神器升级数据",20)
	if not _tData then
	 	return
	end
...
--列表排序
function DataArtifactMake:sortAwards(_tAwd)
	-- body
...
--该奖励是否可领
function DataArtifactMake:getIsCanReward(_index)
	-- body
	local tConf = nil
...
--该奖励是否已领取
function DataArtifactMake:getIsRewarded(_index)
...
-- 获取红点方法
function DataArtifactMake:getRedNums()
	local nNums = 0
	for k, v in pairs(self.tConfs) do
