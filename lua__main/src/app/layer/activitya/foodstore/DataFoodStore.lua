...
local DataFoodStore = class("DataFoodStore", function()
	return Activity.new(e_id_activity.foodstore) 
end)

--创建自己(方便管理)
tActivityDataList[e_id_activity.foodstore] = function (  )
	return DataFoodStore.new()
end

function DataFoodStore:ctor()
	-- body
   self:myInit()
end


function DataFoodStore:myInit( )
 	self.tAs = {}				--已领取奖励			
 	self.tConfs = {}			--配置信息
...
-- 读取服务器中的数据
function DataFoodStore:refreshDatasByServer( _tData )
	--dump(_tData,"数据",20)
	if not _tData then
	 	return
	end
	...
	self.nFm = _tData.m or self.nFm -- 玩家duanz次数
...
-- 获取红点方法
function DataFoodStore:getRedNums(_awardsTip)
	local nNums = 0
...
--获取奖励配置数据
function DataFoodStore:getPrizeConfs(  )
	-- body
	return self.tConfs
end
--是否开始领奖
function DataFoodStore:isStartGetPrize(  )
	-- body
...
--获取奖励配置信息
function DataFoodStore:getRankPrizeDatas(  )
	-- body
	local tTable = {}
...
--获取奖品状态
function DataFoodStore:getPrizeStatus( Index )
	-- body
...
function DataFoodStore:updateRankPrizeStatus( )
	-- body
	for k, v in pairs(self.tConfs) do
...
--是否已经领取奖励
function DataFoodStore:isHaveGetPrize(_index)
	-- body
	if not _index then
		return false
	end
...
--是否在奖励范围内
function DataFoodStore:isInGetPrizeRange( _index )
	-- body
...
--根据排名确定等级
function DataFoodStore:getClassifyByRank( _nRank )
	-- body
...
--获取当前玩家的奖励档次
function DataFoodStore:getMyPrizeClassify()
	-- body
	return self:getClassifyByRank(self.nHz) 
end

--活动结算时间
function DataFoodStore:getBalanceTimeStr(  )
	-- body	
