...
local ItemEMGetReward =  require("app.layer.activitya.equipmake.ItemEMGetReward")
local ItemEquipMake = class("ItemEquipMake", function()
...
function ItemEquipMake:ctor()
	...
	self:onResume()
	--注册析构方法
	self:setDestroyHandler("ItemEquipMake",handler(self, self.onItemDestroy))	
end

--初始化控件
function ItemEquipMake:setupViews( )
	self.pLayDesc:setVisible(true)
end

--更新
function ItemEquipMake:updateViews( )
...
--析构方法
function ItemEquipMake:onItemDestroy(  )
end

-- 注册消息
function ItemEquipMake:regMsgs( )
	regMsg(self, gud_refresh_activity, handler(self, self.updateViews))
end

-- 注销消息
function ItemEquipMake:unregMsgs(  )
	unregMsg(self, gud_refresh_activity)
end


function ItemEquipMake:onResume(  )
	self:regMsgs()
end

function ItemEquipMake:onPause(  )
	self:unregMsgs()
end

--设置数据 _data
function ItemEquipMake:setData(_tData)
	if not _tData then
		return
	end
