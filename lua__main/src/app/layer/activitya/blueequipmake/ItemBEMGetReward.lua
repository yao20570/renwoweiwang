...
local ItemBEMGetReward = class("ItemBEMGetReward", function()
	return ItemActGetReward.new()
end)

function ItemBEMGetReward:ctor(  )
	self:myInit()
	self:setupViews()
	self:onResume()
	--注册析构方法
	self:setDestroyHandler("ItemBEMGetReward",handler(self, self.onItemBEMGetRewardDestroy))	
end


function ItemBEMGetReward:myInit()
	self.nCId 		= 0 --序号
	self.nEn  		= 0   --件数
...
function ItemBEMGetReward:regMsgs(  )
end

function ItemBEMGetReward:unregMsgs(  )
end

function ItemBEMGetReward:onResume(  )
	self:regMsgs()
end

function ItemBEMGetReward:onPause(  )
	self:unregMsgs()
end


function ItemBEMGetReward:onItemBEMGetRewardDestroy(  )
	self:onPause()
end

function ItemBEMGetReward:setupViews()
	--条件
...
function ItemBEMGetReward:updateViews()
	...
	local tAct = Player:getActById(e_id_activity.blueequipmake)
...
function ItemBEMGetReward:setItemData(_tLogData)
	if _tLogData then
...
--领取点击事件
function ItemBEMGetReward:onGetClicked( pView )
	local tAct = Player:getActById(e_id_activity.blueequipmake)
	--可以领取
	if tAct:getIsCanReward(self.nCId) then
		SocketManager:sendMsg("blueequipmake", {self.nCId}, function(__msg)
			-- dump(__msg, "__msg")
		    if  __msg.head.state == SocketErrorType.success then 
		        if __msg.head.type == MsgType.blueequipmake.id then
