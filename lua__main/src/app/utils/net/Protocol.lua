...
local function _getProtocol(__type)
	assert(MsgType[__type], "Can not find ".." protocol in MsgType:"..__type.."!")
	assert(_p[__type], "Can not find ".." protocol in method:"..__type.."!")
	local __oldTable = _p[__type]
	local __newTable = {} 
	__newTable.method = __type
	__newTable.ID = -MsgType[__type].id
...
function Protocol.getSend(__type)
	return _getProtocol(__type)
end

function Protocol.getSendByReceive(__methodCode)
	__methodCode = __methodCode - 1000
	return Protocol.getSend(__methodCode)
end
